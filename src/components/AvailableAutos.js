...
const DATA = [
    { id: 1, name: 'BPX-2611', distance: '5 kms', ratings: 5, departAt: '10.60 AM' },
    { id: 2, name: 'BPX-8198', distance: '10 kms', ratings: 5, departAt: '10.60 AM' },
    { id: 3, name: 'BPX-5536', distance: '15 kms', ratings: 5, departAt: '10.60 AM' },
    { id: 4, name: 'BPX-5536', distance: '15 kms', ratings: 4, departAt: '10.60 AM' },
    { id: 5, name: 'BPX-5536', distance: '15 kms', ratings: 5, departAt: '10.60 AM' },
];
    ...
    const Driver = ({ item, selected, setSelected }) => {
        return (
            <TouchableOpacity
                ...
                style={[styles.auto, { backgroundColor: selected == item.id ? '#fbb74d' : '#e1e1e1' }]}
            >
                <Avatar style={[tw`w-80px ml-4 h-80px  rounded-full`, { backgroundColor: selected == item.id ? '#000000' : '#ffffff' }]} />
                <View style={tw`ml-8`}>
                    <Text style={tw`text-20px font-400  text-black font-bold`}>
                        {item.name}
                    </Text>
                    <Text style={tw`text-16px text-black`}>
                        Distance : {item.distance}
                    </Text>
                    <Text style={tw`text-black`}>
                        Ratings : {item.ratings}
                    </Text>
                    <Text style={tw`text-16px font-400 mb-2 text-black `}>
                        Depart At : {item.departAt}
                    </Text>
                </View>
            </TouchableOpacity>
        );
        ...
        <SafeAreaView style={[tw`flex-1 bg-white `, {}]}>
            ...
            >
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => <Driver item={item} selected={selected} setSelected={setSelected} />
                    }
            ...
            <TouchableOpacity
                ...
                style={[tw` m-4 p-2 w-40 rounded-full `, styles.nextButton, { backgroundColor: selected ? 'green' : 'gray' }]}>
